...
# Handle all the special pages that are not in the url table, but with fixed urls
    if ( $path eq "" ) {
        ...
        # Main page
        {
            ...
            my $sth_grp = $dbh->prepare_cached(
"SELECT g.id, g.name, u.path FROM grp g INNER JOIN url u ON u.id = g.id AND u.type = 'group' AND p_id = ? ORDER BY g.name ASC"
            );
            my $sth_grp_root = $dbh->prepare_cached(
"SELECT g.id, g.name, u.path FROM grp g INNER JOIN url u ON u.id = g.id AND u.type = 'group' AND p_id IS NULL ORDER BY g.name ASC"
            );
            my $sth_node = $dbh->prepare_cached(
"SELECT n.id, n.name, u.path, n.path FROM node n INNER JOIN url u ON u.id = n.id AND u.type = 'node' AND n.grp_id = ? ORDER BY n.name ASC"
            );

            $template_params{GROUPS} =
              _get_params_groups( $path, $dbh, $sth_grp, $sth_grp_root,
                $sth_node, undef, $graph_ext );
...
"SELECT nu.path, n.name, su.path, s.name, d.critical, d.warning, d.unknown FROM ds d
        ...
        while ( my ($_service) = $sth_cat->fetchrow_array ) {
            push @services,
              _get_params_services_by_name( $dbh, $_service, $time,
                $graph_ext );
        }
    ...
    my $sth_url =
      $dbh->prepare_cached("SELECT id, type FROM url WHERE path = ?");
...
"SELECT n.id, n.name, u.path, n.path FROM node n INNER JOIN url u ON u.id = n.id AND u.type = 'node' AND n.grp_id = ? ORDER BY n.name ASC"
        ...
        my $sth_p_id =
          $dbh->prepare_cached("SELECT g.p_id FROM grp g WHERE g.id = ?");
        $sth_p_id->execute($id);
        ...
        if ($comparison) {
            ...
            while ( my ($cat_name) = $sth_cat->fetchrow_array ) {
                push @{ $template_params{CATEGORIES} },
                  _get_params_services_for_comparison( $path, $dbh, $cat_name,
                    $id, $graph_ext, $comparison );
            }
        }
        else {
            # Emit group template
            ...
            $template_params{COMPARE} = 1
              if 1 < scalar
              grep { defined( $_->{'NCATEGORIES'} ) && $_->{'NCATEGORIES'} }
              @{ $template_params{GROUPS} };
        }
        ...
        while ( my ($_category_name) = $sth_category->fetchrow_array ) {
            push @$categories,
              _get_params_services( $path, $dbh, $_category_name, undef, $id,
                $graph_ext );
        }
        ...
        my ( $graph_name, $graph_title, $graph_info, $multigraph, $category,
            $state_warning, $state_critical )
          = $sth->fetchrow_array();

        $sth = $dbh->prepare_cached(
            "SELECT category FROM service_categories WHERE id = ?");
        $sth->execute($id);
        my ($graph_category) = $sth->fetchrow_array();

        $sth = $dbh->prepare_cached(
"SELECT n.id FROM node n INNER JOIN service s ON s.node_id = n.id WHERE s.id = ?"
        );
        $sth->execute($id);
        ...
        $template_params{PEERS} = [
            map {
                ( my $name = basename( $_->{"URLX"}, ".html" ) ) =~ tr/_/ /;
                {
                    NAME => $name,
                    LINK => $multigraph ? ( "../" . $_->{URLX} ) : $_->{URLX},
                }
              } @{
                _get_params_services(
                    dirname($path), $dbh,     $graph_category,
                    $graph_parent,  $node_id, $graph_ext
                )->{SERVICES}
              }
        ];
        $template_params{LARGESET}    = 1;
        ...
        if ($multigraph) {
            ...
            my @categories = (
                _get_params_services(
                    $path,       $dbh,     $graph_category,
                    $graph_name, $node_id, $graph_ext
                )
            );
            $template_params{CATEGORIES}  = \@categories;
            $template_params{NCATEGORIES} = scalar(@categories);
        ...
        for my $t (@times) {
            ...
            $service_template_params{"ZOOM$t"} =
                "/dynazoom.html?cgiurl_graph=$cgi_graph_url"
              . "&plugin_name=$path&size_x=800&size_y=400&$epoch";
            $service_template_params{"IMG$t"} =
              $cgi_graph_url . "$path-$t.$graph_ext";
        }
  ...
  RENDERING:
    ...
    if ( $output_format eq "html" ) {
        ...
        my $template = HTML::Template::Pro->new(
            filename =>
"$Munin::Common::Defaults::MUNIN_CONFDIR/templates/$template_filename",
            loop_context_vars => 1,
        );
...
sub _get_params_groups {
    my ( $path, $dbh, $sth_grp_normal, $sth_grp_root, $sth_node, $g_id,
        $graph_ext )
      = @_;
    ...
    foreach my $row (@$_sth_grp_data) {
        ...
        my $_groups =
          _get_params_groups( $path, $dbh, $sth_grp_normal, $sth_grp_root,
            $sth_node, $_g_id, $graph_ext );
        my $_compare_groups =
          scalar grep { defined( $_->{'NCATEGORIES'} ) && $_->{'NCATEGORIES'} }
          @$_groups;
        push @$groups, {
            NAME    => $_name,
            URL     => "$_path/",
            ...
            R_PATH => '',
            PATH =>
              [ { PATH => '..', PATHNAME => undef, }, url_to_path($_path), ],
        };
    }

    # Add the nodes
    $sth_node->execute($g_id);
    while ( my ( $_n_id, $_name, $_path, $_node_path ) =
        $sth_node->fetchrow_array )
    {
        my $sth = $dbh->prepare_cached(
"SELECT DISTINCT sc.category FROM service s INNER JOIN service_categories sc ON sc.id = s.id WHERE s.node_id = ? ORDER BY sc.category ASC"
        );
        $sth->execute($_n_id);
        ...
        while ( my ($_category_name) = $sth->fetchrow_array ) {
            my $category =
              _get_params_services( $path, $dbh, $_category_name, undef,
                $_n_id, $graph_ext );
...
sub _get_params_services_for_comparison {
    my ( $basepath, $dbh, $category_name, $grp_id, $graph_ext, $comparison ) =
      @_;
...
# Get all possible services with the specified category under the specified group
    ...
    while ( my ( $service_name, $service_title ) = $sth_srv->fetchrow_array ) {
        ...
        while ( my ( $node_name, $node_url, $srv_url, $srv_label ) =
            $sth_node->fetchrow_array )
        {
            my $_srv_url = "$srv_url.html" if defined $srv_url;
            my $_img_url = "/$srv_url-$comparison.$graph_ext"
              if defined $srv_url;
            push @nodes,
              {
                R_PATH   => '',
...
"SELECT s.id, s.service_title as service_title, s.subgraphs as subgraphs, u.path AS url,
    ...
    while (
        my (
            $_s_id, $_service_title, $_subgraphs, $_url, $_node_name,
            $_node_url
        )
        = $sth->fetchrow_array
      )
...
sub _get_params_services {
    my ( $base_path, $dbh, $category_name, $multigraph_parent, $node_id,
        $graph_ext )
      = @_;
...
"SELECT s.id, s.name, s.service_title as service_title, s.subgraphs as subgraphs, u.path AS url,
    ...
    while (
        my (
            $_s_id,      $_s_name, $_service_title,
            $_subgraphs, $_url,    $_state_warning,
            $_state_critical
        )
        = $sth->fetchrow_array
      )
    {
        ...
        push @$services,
          {
            NAME => $_service_title,
            URLX => substr( $_url, 1 + length($base_path) )
              . ( $_subgraphs ? "/" : ".html" ),
...
sub _get_params_fields {
    ...
    while (
        my (
            $_ds_name, $_ds_s_warn, $_ds_s_crit, $_ds_graph, $_ds_label,
            $_ds_type, $_ds_warn,   $_ds_crit,   $_ds_info
        )
...
sub get_param {
    ...
    my ($value) =
      $dbh->selectrow_array( "SELECT value FROM param WHERE name = ?",
        undef, ($param) );
