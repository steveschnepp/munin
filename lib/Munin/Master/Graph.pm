...
sub handle_request {
    ...
    if ( $path !~
m/^\/(.*)-(hour|day|week|month|year|pinpoint=(\d+),(\d+))\.(svg|json|csv|xml|png|[a-z]+)$/
      )
    {
        # We don't understand this URL
        print "HTTP/1.0 404 Not found\r\n";
        print $cgi->header( "-X-Reason" => "invalid URL: $path", );
        return;
    }
    ...
    if ( !$CONTENT_TYPES{$format} ) {
        ...
        print "HTTP/1.0 404 Not found\r\n";
        print $cgi->header( "-X-Reason" => "invalid format $format", );
        return;
    }
...
# Note that we reconnect for _each_ request. This is to avoid old data when the DB "rotates"
    ...
    if ( !defined $id ) {
        ...
        # Not found
        print "HTTP/1.0 404 Not found\r\n";
        print $cgi->header( "-X-Reason" => "'$graph_path' Not Found in DB", );
        return;
    }
    ...
    $sth = $dbh->prepare_cached(
        "SELECT value FROM service_attr WHERE id = ? and name = ?");
    ...
    if ( !defined $graph_printf ) {
        ...
        $graph_printf =
          ( $graph_args =~ /--base\s+1024/ ) ? "%7.2lf" : "%6.2lf";
    }
    ...
    while (
        my (
            $_rrdname,  $_label,    $_rrdfile,      $_rrdfield,
            $_rrdalias, $_rrdcdef,  $_color,        $_drawtype,
            $_printf,   $_negative, $_has_negative, $_lastupdated,
        )
        = $sth->fetchrow_array()
      )
    {
...
"($_alias_service $_alias_ds) = ($_rrdfile $_rrdfield, $_lastupdated)";
        ...
        push @rrd_def,
          "DEF:avg_$real_rrdname=" . $_rrdfile . ":" . $_rrdfield . ":AVERAGE";
        push @rrd_def,
          "DEF:min_$real_rrdname=" . $_rrdfile . ":" . $_rrdfield . ":MIN";
        push @rrd_def,
          "DEF:max_$real_rrdname=" . $_rrdfile . ":" . $_rrdfield . ":MAX";

        # Handle an eventual cdef
        if ($_rrdcdef) {

            # Sustitute the name with the real RRD
            push @rrd_def, "CDEF:avg_$_rrdname="
              . expand_cdef( $_rrdname, $_rrdcdef, "avg_$real_rrdname" );
            push @rrd_def, "CDEF:min_$_rrdname="
              . expand_cdef( $_rrdname, $_rrdcdef, "min_$real_rrdname" );
            push @rrd_def, "CDEF:max_$_rrdname="
              . expand_cdef( $_rrdname, $_rrdcdef, "max_$real_rrdname" );
        }
        ...
        $_drawtype = $field_number ? "STACK" : "AREA"
          if $_drawtype eq "AREASTACK";
        $_drawtype = $field_number ? "STACK" : "LINE"
          if $_drawtype eq "LINESTACK";
      ...
      # ... But we did still want to compute the related DEF & CDEF
        ...
        push @rrd_gfx,
          "COMMENT:$label_as_white";  # Rewind the line, to have \r after the \l
        ...
        push @rrd_gfx_negatives, "$_drawtype:avg_n_$_rrdname#$_color"
          if $_negative;

        $lastupdated = $_lastupdated
          if !defined $lastupdated
          || ( $_lastupdated && $_lastupdated > $lastupdated );
    }
    continue {
    ...
    print $cgi->header( "-Content-type" => $CONTENT_TYPES{$format}, )
...
'LEGEND:7:DejaVuSansMono,DejaVu Sans Mono,DejaVu LGC Sans Mono,Bitstream Vera Sans Mono,monospace',
    ...
    my @rrd_cmd = (
        $rrd_fh->filename, @rrd_header,        @rrd_def,
        @rrd_gfx,          @rrd_gfx_negatives, @rrd_legend,
    );

    my $err = RRDs_graph_or_dump( $format, @rrd_cmd, );
    if ($err) {
        INFO "'" . join( "' \\\n'", @rrd_cmd ) . "'";
...
sub expand_cdef {
    ...
    $_rrdcdef =~ s/(?<=[,=])$_rrdname(?=[,=]|$)/$real_rrdname/g
      ;    # ?<= is lookbehind, ?= is lookforward
    $_rrdcdef =~
      s/^$_rrdname(?=[,=]|$)/$real_rrdname/g;    # Also handle the first element
    DEBUG "=$_rrdcdef";
    return $_rrdcdef;
}
...
sub RRDs_graph {
    ...
    my $chld_pid =
      open3( $chld_out, $chld_in, $chld_err, "rrdtool", "graphv", @_ );
...
sub RRDs_graph_or_dump {
    ...
    DEBUG "[DEBUG] \n\nrrdtool xport '" . join( "' \\\n\t'", @xport ) . "'\n";
    my ( $start, $end, $step, $nb_vars, $columns, $values ) =
      RRDs::xport(@xport);
    if ( $fileext eq "CSV" ) {
